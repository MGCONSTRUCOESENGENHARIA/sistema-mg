...
export default function FolhasPage() {
  ...
  async function salvar() {
    ...
    const { data: up, error: upErr } = await supabase.storage.from('folhas-ponto').upload(nome, arquivo)
    ...
    // Salvar no banco
    const { error } = await supabase.from('folhas_ponto').insert({
    ...
    if (error) { setMsg('⚠️ Erro ao salvar: ' + error.message); setUploading(false); return }
  ...
  async function remover(id: string, fotoUrl: string) {
    ...
    if (path) await supabase.storage.from('folhas-ponto').remove([path])
      ...
      <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 12, padding: 20, marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px,1fr))', gap: 12, marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {(['ARMAÇÃO', 'CARPINTARIA'] as const).map(eq => (
                <button key={eq} onClick={() => setForm(f => ({ ...f, equipe: eq }))}
                  style={{ flex: 1, padding: '8px 6px', borderRadius: 8, border: '1.5px solid', cursor: 'pointer', fontSize: 12, fontWeight: 600,
          ...
          <div>
            ...
            <button onClick={() => setForm(f => ({ ...f, tem_diaria_extra: !f.tem_diaria_extra }))}
              style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1.5px solid', cursor: 'pointer', fontSize: 13, fontWeight: 700,
              ...
              {form.tem_diaria_extra ? '⚡ Sim — tem extra' : 'Não tem extra'}
            </button>
          </div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
          <div>
            ...
            <input value={form.observacao} onChange={e => setForm(f => ({ ...f, observacao: e.target.value }))}
              placeholder="Ex: descarga de aço, caminhão..."
              style={{ width: '100%', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: '8px 10px', fontSize: 13, outline: 'none' }} />
          </div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#6b7280', display: 'block', marginBottom: 4 }}>Foto da folha *</label>
            <input ref={fileRef} type="file" accept="image/*" capture="environment" onChange={handleFile}
              style={{ display: 'none' }} />
            <button onClick={() => fileRef.current?.click()}
              style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1.5px dashed #d1d5db', background: '#f9fafb', color: '#6b7280', cursor: 'pointer', fontSize: 13 }}>
              {arquivo ? `📄 ${arquivo.name}` : '📷 Selecionar / Tirar foto'}
            </button>
          </div>
        </div>
        ...
        {preview && (
          <div style={{ marginBottom: 14 }}>
            <img src={preview} alt="preview" style={{ maxHeight: 200, borderRadius: 8, border: '1px solid #e5e7eb', objectFit: 'contain' }} />
          </div>
        )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px,1fr))', gap: 12, padding: 16 }}>
              {items.map(f => (
                <div key={f.id} style={{ border: `2px solid ${f.tem_diaria_extra ? '#fbbf24' : '#e5e7eb'}`, borderRadius: 10, overflow: 'hidden', position: 'relative' }}>
                  {/* Badge extra */}
                  {f.tem_diaria_extra && (
                    <div style={{ position: 'absolute', top: 6, right: 6, background: '#f59e0b', color: 'white', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 20, zIndex: 1 }}>
                      ⚡ EXTRA
                    </div>
                  )}
                  {/* Foto */}
                  <div onClick={() => setFotoAberta(f.foto_url)} style={{ cursor: 'pointer' }}>
                    <img src={f.foto_url} alt="folha" style={{ width: '100%', height: 120, objectFit: 'cover', display: 'block' }} />
                  </div>
                  {/* Info */}
