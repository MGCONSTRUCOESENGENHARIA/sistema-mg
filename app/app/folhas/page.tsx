...
export default function PresencaPage() {
  ...
  function abrirModal(funcId: string, funcNome: string, data: string) {
    ...
    if (atual) {
      ...
      setFormFracao2(atual.obra2_id ? String(atual.fracao2 || 0.5) : '')
    } else {
  ...
  async function salvar() {
    ...
    const fracao1 = parseFloat(formFracao) || 1
    const fracao2 = formObra2 ? (parseFloat(formFracao2) || 0.5) : null
    const totalFracoes = fracao1 + (fracao2 || 0)

    if (formObra2 && totalFracoes !== 1) {
      setFormErro('Quando tiver 2 obras, a soma das frações precisa dar 1 diária.')
      return
    }
    ...
    const payload: any = {
      ...
      obra_id: formObra || null, fracao: fracao1,
      obra2_id: formObra2 || null, fracao2: fracao2,
    }
    try {
            ...
            <div style={{ padding:20, display:'flex', flexDirection:'column', gap:14 }}>
              ...
              {(formTipo==='NORMAL'||formTipo==='SABADO_EXTRA') && (
                <>
                    ...
                    <div>
                      ...
                      <select style={{ width:'100%', border:'1px solid #d1d5db', borderRadius:8, padding:'8px 10px', fontSize:13 }}
                        value={formObra2} onChange={e => {
                          ...
                          setFormObra2(value)
                          setFormFracao2(value ? '0.5' : '')
                          if (value && formFracao === '1') setFormFracao('0.5')
                        }}>
                  ...
                  {formObra2 && <div style={{ background:'#eff6ff', border:'1px solid #bfdbfe', borderRadius:6, padding:'8px 12px', fontSize:11, color:'#1e40af' }}>2 obras no mesmo dia = 0,5 + 0,5 = 1 diária inteira</div>}
                </>
              )}
            </div>
