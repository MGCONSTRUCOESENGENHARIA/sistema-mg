...
function calcularValorPassagemDaPresenca(p: any, funcId: string, passDB: any[]) {
  ...
  return {
    valor: valor1 + valor2,
    dias: fracao1 + fracao2,
    valorPraFrente:
      fop2?.tipo_passagem === "PRA FRENTE"
        ? Number(fop2.valor_passagem || 0)
        : fop1?.tipo_passagem === "PRA FRENTE"
          ? Number(fop1.valor_passagem || 0)
          : 0,
  };
}
...
export default function PassagemCafePage() {
  ...
  async function carregar() {
    ...
    if (comp?.id) {
      ...
      while (true) {
        const { data: page } = await supabase
          .from("presencas")
          .select("id,funcionario_id,data,tipo,fracao,fracao2,obra_id,obra2_id")
    ...
    const resultado: Linha[] = funcs.map((func: any) => {
      ...
      presFunci.forEach((p) => {
        ...
        diasTrabalhados += calculo.dias;
        valorGasto += calculo.valor;

        if (calculo.valorPraFrente > 0) {
          ultimoValorPraFrente = calculo.valorPraFrente;
        }
      });
      const tiposFunc =
        passDB?.filter((x) => x.funcionario_id === func.id) || [];
  ...
  return (
    ...
    >
      ...
      >
        Passagem & Café — {equipe} ·{" "}
        {quinzena === 1 ? "1ª Quinzena" : "2ª Quinzena"}
      </h1>
      <p style={{ fontSize: 12, color: "#6b7280", marginBottom: 8 }}>
        ...
        são editáveis · Café = R$10/dia trabalhado
      </p>
      {msg && (
        <div
          style={{
      ...
      >
        {[
          {
            label: "Total a Receber de Passagem",
            val: totPassagem,
            color: "#1e40af",
          },
          {
            label: `Total Café da Manhã (R$${CAFE_DIA}/dia)`,
            val: totCafe,
            color: "#166534",
          },
                  ...
                  <tr key={l.func_id} style={{ background: bg }}>
                    ...
                    <td
                      style={{
                        padding: "4px 6px",
                        textAlign: "center",
                        background: isPraFrente ? "#fefce8" : "#f9fafb",
                      }}
                    >
                      {isPraFrente ? (
                        <input
                    ...
                    >
                      {l.valor_gasto_quinzena > 0
                        ? formatR$(l.valor_gasto_quinzena)
                        : "—"}
                    </td>
                    <td
                      style={{
                    ...
                    <td
                      style={{
                        ...
                        textAlign: "right",
                        fontSize: 12,
                        color: isPraFrente ? "#1e40af" : "#9ca3af",
                      }}
                    >
