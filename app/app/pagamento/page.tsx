              ...
              {linhas.map((l, fi) => {
                ...
                return (
                  <tr key={l.func_id} style={{ background: bg }}>
                    ...
                    <td style={{ padding: '4px 4px', textAlign: 'center', background: cores.bg }}>
                      <select
                        value={tipo}
          ...
          <div style={{ background: 'white', borderRadius: 16, width: '100%', maxWidth: 440, overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: 20 }}>
              {[
                ...
                { label: '(-) Desc. Acerto', val: '-' + formatR$(modalFunc.ed.desc_acerto || 0), color: '#dc2626' },
                { label: '(-) Desc. Pensão', val: '-' + formatR$(modalFunc.ed.desc_pensao || 0), color: '#dc2626' },
                { label: '(-) Desc. DSR', val: '-' + formatR$(modalFunc.calc.dsr || 0), color: '#dc2626' },
                { label: '(-) Sindicato', val: '-' + formatR$(modalFunc.calc.sindicato || 0), color: '#dc2626' },
                { label: '(-) INSS', val: '-' + formatR$(modalFunc.calc.inss || 0), color: '#dc2626' },
              ].map((item, i) => (
