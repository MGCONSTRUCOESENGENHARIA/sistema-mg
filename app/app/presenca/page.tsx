                ...
                <button onClick={salvar} disabled={salvando} style={{ padding:'7px 16px', borderRadius:8, border:'none', background:'#1a3a5c', color:'#fff', cursor:'pointer', fontSize:13, fontWeight:600 }}>
                  {salvando?'Salvando...':'Confirmar'}
                          ...
                          {editandoCell === `${func.id}|${key}` ? (
                              ...
                              <input
                                ...
                                onKeyDown={async e => {
                                  if (e.key === 'Enter' || e.key === 'Tab') {
                                    ...
                                    // Tab = próximo funcionário mesmo dia
                                ...
                                style={{ width:'100%', border:'2px solid #7c3aed', borderRadius:4, padding:'2px 4px', fontSize:10, outline:'none', background:'white', textAlign:'center' }}
                              />
                              {sugestoes.length > 0 && (
                                <div style={{ position:'absolute', top:'100%', left:0, zIndex:100, background:'white', border:'1px solid #e5e7eb', borderRadius:6, boxShadow:'0 4px 12px rgba(0,0,0,.15)', minWidth:140, maxWidth:200 }}>
                                  {sugestoes.map((s,si) => (
                                    <div key={si}
                                      onMouseDown={async e => {
                                        e.preventDefault()
                                        setEditVal(s); setSugestoes([])
                                        await salvarCelulaTexto(func.id, key, s)
                                        setEditandoCell(null)
                                      }}
                                      style={{ padding:'6px 10px', fontSize:11, cursor:'pointer', borderBottom:'1px solid #f3f4f6', color:'#1f2937', fontWeight:500 }}
                            ...
                            <span
                              onClick={() => {
                                setEditandoCell(`${func.id}|${key}`)
                                setEditVal(labelParaTexto(p))
                                setSugestoes([])
                              }}
