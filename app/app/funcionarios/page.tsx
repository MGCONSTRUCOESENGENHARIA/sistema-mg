...
export default function FuncionariosPage() {
  ...
  async function salvarNovo() {
    if (!form.nome.trim() || !form.equipe) {
      alert('Preencha nome e equipe.')
      return
    }
  ...
  if (ordemDiaria === 'asc') {
    lista = [...lista].sort((a, b) => a.valor_diaria - b.valor_diaria)
  }

  if (ordemDiaria === 'desc') {
    lista = [...lista].sort((a, b) => b.valor_diaria - a.valor_diaria)
  }
      ...
      <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 12, padding: '14px 18px', marginBottom: 16 }}>
        <div style={{ position: 'relative', marginBottom: 12 }}>
          <input
            ...
            style={{ width: '100%', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: '8px 12px', fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
          />
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: 4 }}>
            {(['TODOS', 'ARMAÇÃO', 'CARPINTARIA'] as const).map(eq => (
              <button key={eq} style={btnEq(eq)} onClick={() => setEquipe(eq)}>
                {eq === 'TODOS' ? 'Todos' : eq}
              </button>
            ))}
          </div>
          ...
          <select
            value={ordemDiaria}
            onChange={e => setOrdemDiaria(e.target.value as any)}
            style={{ border: '1.5px solid #e5e7eb', borderRadius: 8, padding: '6px 10px', fontSize: 13, color: '#374151', outline: 'none' }}
          >
          ...
          <div style={{ background: 'white', borderRadius: 16, width: '100%', maxWidth: 480, overflow: 'hidden' }}>
            ...
            <div style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                ...
                <input value={modal.nome} onChange={e => setModal(m => m ? { ...m, nome: e.target.value } : m)} style={{ width: '100%', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: '8px 10px', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
              </div>

              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: '#6b7280', display: 'block', marginBottom: 4 }}>Função</label>
                <input value={modal.funcao || ''} onChange={e => setModal(m => m ? { ...m, funcao: e.target.value } : m)} style={{ width: '100%', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: '8px 10px', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: '#6b7280', display: 'block', marginBottom: 4 }}>Diária</label>
                  <input type="number" value={modal.valor_diaria} onChange={e => setModal(m => m ? { ...m, valor_diaria: parseFloat(e.target.value) || 0 } : m)} style={{ width: '100%', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: '8px 10px', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
                </div>

                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: '#6b7280', display: 'block', marginBottom: 4 }}>Salário Base</label>
                  <input type="number" value={modal.salario_base} onChange={e => setModal(m => m ? { ...m, salario_base: parseFloat(e.target.value) || 0 } : m)} style={{ width: '100%', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: '8px 10px', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
                </div>
              </div>

              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: '#6b7280', display: 'block', marginBottom: 4 }}>Equipe</label>
                <select value={modal.equipe} onChange={e => setModal(m => m ? { ...m, equipe: e.target.value } : m)} style={{ width: '100%', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: '8px 10px', fontSize: 13, outline: 'none' }}>
              ...
              <div>
                ...
                <select value={modal.empresa} onChange={e => setModal(m => m ? { ...m, empresa: e.target.value } : m)} style={{ width: '100%', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: '8px 10px', fontSize: 13, outline: 'none' }}>
                  {Object.keys(EMPRESAS).map(e => (
                    <option key={e} value={e}>{e}</option>
                  ))}
                </select>
            ...
            <div style={{ padding: '14px 22px', borderTop: '1px solid #f3f4f6', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => { excluir(modal.id, modal.nome); setModal(null) }} style={{ padding: '8px 18px', borderRadius: 8, border: 'none', background: '#fef2f2', color: '#dc2626', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>🗑 Excluir</button>
          ...
          <div style={{ background: 'white', borderRadius: 16, width: '100%', maxWidth: 480, overflow: 'hidden' }}>
            ...
            <div style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 10 }}>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: '#6b7280', display: 'block', marginBottom: 4 }}>Nome *</label>
                  <input value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))} style={{ width: '100%', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: '8px 10px', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
                </div>

                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: '#6b7280', display: 'block', marginBottom: 4 }}>Equipe *</label>
                  <select value={form.equipe} onChange={e => setForm(f => ({ ...f, equipe: e.target.value }))} style={{ width: '100%', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: '8px 10px', fontSize: 13, outline: 'none' }}>
                    <option>ARMAÇÃO</option>
                    <option>CARPINTARIA</option>
                  </select>
                </div>
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: '#6b7280', display: 'block', marginBottom: 4 }}>Função</label>
                <input value={form.funcao} onChange={e => setForm(f => ({ ...f, funcao: e.target.value }))} style={{ width: '100%', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: '8px 10px', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: '#6b7280', display: 'block', marginBottom: 4 }}>Diária (R$)</label>
                  <input type="number" value={form.valor_diaria} onChange={e => setForm(f => ({ ...f, valor_diaria: e.target.value }))} style={{ width: '100%', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: '8px 10px', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
                </div>

                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: '#6b7280', display: 'block', marginBottom: 4 }}>Salário Base (R$)</label>
                  <input type="number" value={form.salario_base} onChange={e => setForm(f => ({ ...f, salario_base: e.target.value }))} style={{ width: '100%', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: '8px 10px', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
                </div>
              </div>

              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: '#6b7280', display: 'block', marginBottom: 4 }}>Empresa</label>
                <select value={form.empresa} onChange={e => setForm(f => ({ ...f, empresa: e.target.value }))} style={{ width: '100%', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: '8px 10px', fontSize: 13, outline: 'none' }}>
