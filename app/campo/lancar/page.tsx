      ...
      {tela==='obra' && (
        <>
          <div style={css.body}>
            ...
            <span style={css.lbl}>Digite para buscar</span>
            <input
              style={css.inp}
              value={buscaObra}
              placeholder="Ex: Barreiro, Savassi..."
              onChange={ev => { setBuscaObra(ev.target.value); setObraId('') }}
            />
            {buscaObra.length > 0 && (
              <div style={{background:'white',border:'2px solid #cbd5e1',borderRadius:12,marginTop:6,overflow:'hidden'}}>
                {obras.filter(o=>o.nome.toLowerCase().includes(buscaObra.toLowerCase())).slice(0,8).map(o=>(
                  <div key={o.id}
                    style={{padding:'14px 16px',fontSize:15,color:'#111',borderBottom:'1px solid #f1f5f9',cursor:'pointer',background:obraId===o.id?'#dbeafe':'white'}}
                    onClick={()=>{ setObraId(o.id); setBuscaObra(o.nome) }}>
                    {o.nome}
                  </div>
                ))}
              </div>
            )}
            {obraId && <div style={{marginTop:12,padding:'12px 16px',background:'#dbeafe',borderRadius:12,fontWeight:700,color:'#1e3a8a'}}>✓ {obras.find(o=>o.id===obraId)?.nome}</div>}
          </div>
        ...
        <>
          <div style={css.body}>
            ...
            {funcsDisp.length===0 && funcs.length>0 ? (
              ...
              <>
                <input style={css.inp} value={buscaFunc} placeholder="Buscar funcionário..."
                  onChange={ev=>setBuscaFunc(ev.target.value)} />
                {buscaFunc.length>0 && (
                  <div style={{background:'white',border:'2px solid #cbd5e1',borderRadius:12,marginTop:6,overflow:'hidden'}}>
                    {funcsDisp.filter(f=>!presentesIds.includes(f.id)&&f.nome.toLowerCase().includes(buscaFunc.toLowerCase())).slice(0,8).map(f=>(
                      <div key={f.id}
                        style={{padding:'14px 16px',fontSize:15,color:'#111',borderBottom:'1px solid #f1f5f9',cursor:'pointer'}}
                        onClick={()=>{ setPresentesIds(prev=>[...prev,f.id]); setBuscaFunc('') }}>
                        {f.nome}
                      </div>
                    ))}
                  </div>
                ...
                <div style={{display:'flex',flexWrap:'wrap',gap:8,marginTop:12}}>
                  {presentesIds.map(id=>{ const f=funcs.find(x=>x.id===id); if(!f) return null; return (
                    <div key={id} style={{display:'inline-flex',alignItems:'center',gap:6,padding:'8px 12px',borderRadius:20,background:'#dbeafe',fontSize:14,fontWeight:600,color:'#1e40af'}}>
                      {f.nome.split(' ')[0]}
                      <span style={{fontSize:20,color:'#93c5fd',cursor:'pointer',lineHeight:1}}
                        onClick={()=>setPresentesIds(prev=>prev.filter(i=>i!==id))}>×</span>
                    </div>
                  )})}
                </div>
              </>
